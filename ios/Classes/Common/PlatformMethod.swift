...
struct NullaryPlatformMethod<Context> {
    ...
    func call(in context: Context, arguments args: Any?, completion: @escaping FlutterResult) {
        guard args == nil
            ...
            let result = FlutterError(code: "500", message: name, details: "no arguments required")
            completion(result)
            return
        }
                ...
                let result = message
                    .map { message -> Any in
                        do {
                            ...
                            return FlutterError(code: "500", message: name, details: "unknown error")
                        }
                    }
                completion(result)
...
struct UnaryPlatformMethod<Context, Args: SwiftProtobuf.Message> {
    ...
    func call(in context: Context, arguments args: Any?, completion: @escaping FlutterResult) {
        guard let data = (args as? FlutterStandardTypedData)?.data
            ...
            let result = FlutterError(code: "500", message: name, details: "an argument of type \"\(Args.self)\" is required")
            completion(result)
            return
        }
        ...
        do {
            ...
            let result = FlutterError(code: "500", message: name, details: "a (de-) serialization error occured: \"\(error)\"")
            completion(result)
        }
    }
}
